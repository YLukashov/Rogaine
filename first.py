...
def rec(cur_time, cur_vertex, visited):
    global point_max
    global route_max
    if cur_vertex == 0 and cur_time <= 180:  # Текущая вершина финиш и мы не затратили более 180 минут
        new_point = count_point(visited)
        if new_point > point_max:
            point_max = new_point
            route_max = visited
    if cur_time >= 180 or len(visited) > MAX_VISIT:
        return
    else:
